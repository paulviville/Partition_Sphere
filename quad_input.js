...
function create_quads(points, frames)
{
	...
	for(let i = 0; i < points.length; ++i)
	{
		...
		quad_points.push(points[i].clone().addScaledVector(frames[i][0], 0.2).addScaledVector(rand_dir, 0.0005).normalize());
	 	rand_dir = new THREE.Vector3(Math.random(), Math.random(), Math.random());
		quad_points.push(points[i].clone().addScaledVector(frames[i][1], 0.2).addScaledVector(rand_dir, 0.0005).normalize());
		rand_dir = new THREE.Vector3(Math.random(), Math.random(), Math.random());
		quad_points.push(points[i].clone().addScaledVector(frames[i][0], -0.2).addScaledVector(rand_dir, 0.0005).normalize());
		rand_dir = new THREE.Vector3(Math.random(), Math.random(), Math.random());
		quad_points.push(points[i].clone().addScaledVector(frames[i][1], -0.2).addScaledVector(rand_dir, 0.0005).normalize());
